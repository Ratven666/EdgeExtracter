...
class SegmentModelPlotly:
    ...
    def plot(self, model):
        ...
        fig.update_layout(title_text=f"Model {self.model.name}")
    ...
    def __calk_plot_limits(self):
        ...
        min_x, min_y, min_z = model.x_min, model.y_min, model.z_min
        max_x, max_y, max_z = model.x_max, model.y_max, model.z_max
        limits = [max_x - min_x,
                  max_y - min_y,
                  max_z - min_z]
        length = max(limits) / 2
