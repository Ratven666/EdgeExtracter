...
class VMPlotterMPL:
    ...
    def plot(self, voxel_model):
        ...
        for voxel in self.voxel_model:
            ...
            area[i][j][k] = True
            colors[i][j][k][0] = voxel.color[0] / 255.0
            colors[i][j][k][1] = voxel.color[1] / 255.0
            colors[i][j][k][2] = voxel.color[2] / 255.0
    ...
    def __calk_ax_ticks(self):
        ...
        """
        x_ticks = [round(self.voxel_model.x_min + idx*self.voxel_model.step, 4)
                   for idx in range(self.voxel_model.x_count + 1)]
        y_ticks = [round(self.voxel_model.y_min + idx*self.voxel_model.step, 4)
                   for idx in range(self.voxel_model.y_count + 1)]
        z_ticks = [round(self.voxel_model.z_min + idx*self.voxel_model.step, 4)
    ...
    def __calk_indexes(self, voxel):
        ...
        """
        x0 = self.voxel_model.x_min
        y0 = self.voxel_model.y_min
        z0 = self.voxel_model.z_min
