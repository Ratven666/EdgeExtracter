...
from app.indexes.plotters.DEMIndexPlotterPlotly import DEMIndexPlotterPlotly
...
class DEMIndexABC(ABC):
    ...
    def _get_indexes(self, cell):
        ...
        try:
            ...
            return None
        x0 = self.dem_model.voxel_model.x_min
        y0 = self.dem_model.voxel_model.y_min
    ...
    def save_like_img(self, file_path):
        ...
        data = np.where(data == None, min_value, data)

        # Нормируем значения в диапазон [0, 65535]
        data_normalized = (data - np.min(data)) / (np.max(data) - np.min(data)) * 65535

        # Преобразуем в тип uint8
